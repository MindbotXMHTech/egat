...
// ── Health compute ─────────────────────────────────────────────────────────────
function computeHealth(asset, kpi = KPI_PRESETS[asset.id]) {
  const p = kpi
...
// ── Health details ─────────────────────────────────────────────────────────────
export function getHealthDetails(assetId, overrides = {}) {
  const asset = FLEET.find(f=>f.id===assetId)
  if (!asset) return null
  const p    = { ...KPI_PRESETS[assetId], ...overrides }
  const assetForScore = { ...asset, age: overrides.age ?? asset.age, life: overrides.life ?? asset.life }
  const perf     = 100-(p.cpu+p.bw)/2
  const avail    = Math.max(0,100-p.pkt_loss*18)
  const errScore = Math.max(0,100-p.errors*10)
  const ageFact  = Math.max(0,(1-assetForScore.age/assetForScore.life)*100)
  const maint    = Math.max(0,100-p.days_maint/3)
  const score    = computeHealth(assetForScore, p)
  return {
    score, status: statusFromScore(score),
    dimensions:{
...
export function generateHealthTrend(assetId, scoreOverride) {
  const asset = FLEET.find(f=>f.id===assetId)
  const base  = scoreOverride ?? asset?.health ?? 70
